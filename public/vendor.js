...
/******/ 			script.src = __webpack_require__.p + "" + chunkId + "." + ({"0":"app","2":"ng"}[chunkId]||chunkId) + ".bundle.js";
	...
	;(function ($) {
	    ...
	    function startTimeliner(options) {
	        ...
	        function openEvent(eventHeading, eventBody) {
	            ...
	            $(eventHeading).find('a').removeClass('closed').addClass('open').animate({ fontSize: settings.fontOpen }, settings.baseSpeed);
	            $(eventBody).show(settings.speed * settings.baseSpeed);
	        }

	        function closeEvent(eventHeading, eventBody) {

	            $(eventHeading).find('a').animate({ fontSize: settings.fontClosed }, 0).removeClass('open').addClass('closed');
